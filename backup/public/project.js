...
function displayMetrics() {
    ...
    container.innerHTML = `
        ...
        <div class="metric-card">
            <h4>Total Cost (Selected)</h4>
            <div class="value ${getValueClass(formatCurrency(totalCost))}">${formatCurrency(totalCost)}</div>
            <div style="font-size: 0.875rem; margin-top: 0.5rem; color: #7f8c8d;">
                ${currentProject.building_sf ? formatCurrency(avgCostPerSF) + '/SF' : ''}
            </div>
        </div>
        <div class="metric-card">
            <h4>Low Bid Total</h4>
            <div class="value ${getValueClass(formatCurrency(totalLowBid))}">${formatCurrency(totalLowBid)}</div>
            <div style="font-size: 0.875rem; margin-top: 0.5rem; color: #7f8c8d;">
                ${currentProject.building_sf ? formatCurrency(lowBidCostPerSF) + '/SF' : ''}
            </div>
        </div>
        <div class="metric-card">
            <h4>Median Bid Total</h4>
            <div class="value ${getValueClass(formatCurrency(totalMedianBid))}">${formatCurrency(totalMedianBid)}</div>
            <div style="font-size: 0.875rem; margin-top: 0.5rem; color: #7f8c8d;">
                ${currentProject.building_sf ? formatCurrency(medianBidCostPerSF) + '/SF' : ''}
            </div>
        </div>
        <div class="metric-card">
            <h4>Packages</h4>
...
function editPackage(packageId) {
    ...
    document.getElementById('editAmount').value = pkg.selected_amount || '';
...
document.getElementById('editPackageForm').onsubmit = async (e) => {
    ...
    const selected_amount = parseFloat(document.getElementById('editAmount').value);
